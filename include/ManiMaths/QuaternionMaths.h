...
namespace Mani
{
	...
	[[nodiscard]] Vector3<T> convert(const Quat<T>& q) { return { q.x, q.y, q.z }; }

	// https://raw.org/proof/vector-rotation-using-quaternions/
	template<IsNumeric T1, IsNumeric T2 = T1, IsNumeric TReturn = T1>
	[[nodiscard]] Vector3<TReturn> rotate(const Quat<T1>& q, const Vector3<T2>& v)
	{
	...
	template<IsNumeric T>
	[[nodiscard]] Quat<T> fromAxisAngleRad(T angle, Vector3<T> axis)
	{
		constexpr T _2 = 2;
	...
	template<IsNumeric T>
	[[nodiscard]] Quat<T> fromAxisAngleDeg(T angle , Vector3<T> axis)
	{
